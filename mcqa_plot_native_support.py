...
def _gcd_width(values: tuple[int, ...], *, hidden_size: int) -> int:
    filtered = [max(1, min(int(value), int(hidden_size))) for value in values if int(value) > 0]
    if not filtered:
        return int(hidden_size)
    width = filtered[0]
    for value in filtered[1:]:
        width = math.gcd(int(width), int(value))
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--block-resolutions", help="Comma-separated native widths. The GCD defines the atomic partition.")
...
def _format_summary(*, layer: int, atomic_width: int, support_by_var: dict[str, dict[str, object]], best_ot_by_var: dict[str, dict[str, object]]) -> str:
    lines = [
        "MCQA PLOT Native Support Summary",
        f"layer: {int(layer)}",
        f"token_position_id: {DEFAULT_TOKEN_POSITION_ID}",
        f"atomic_width: {int(atomic_width)}",
        "",
    ]
...
def main() -> None:
    ...
    block_resolutions = tuple(_parse_csv_ints(args.block_resolutions) or list(DEFAULT_BLOCK_RESOLUTIONS))
    ...
    atomic_width = _gcd_width(block_resolutions, hidden_size=hidden_size)
    ...
    for layer in layers:
        ...
        cache_spec = base_run._signature_cache_spec(
            ...
            resolution=int(atomic_width),
            resolved_resolution=int(atomic_width),
        ...
        for epsilon in ot_epsilons:
            output_stem = (
                ...
                f"_atomic-{int(atomic_width)}_sig-{str(args.signature_mode)}_eps-{float(epsilon):g}_ot"
            )
        ...
        support_path = layer_dir / (
            f"mcqa_plot_native_support_layer-{int(layer)}_pos-{DEFAULT_TOKEN_POSITION_ID}"
            f"_atomic-{int(atomic_width)}_sig-{str(args.signature_mode)}_support.json"
        )
        summary_path = layer_dir / (
            f"mcqa_plot_native_support_layer-{int(layer)}_pos-{DEFAULT_TOKEN_POSITION_ID}"
            f"_atomic-{int(atomic_width)}_sig-{str(args.signature_mode)}_summary.txt"
        )
        ...
        payload = {
            ...
            "signature_mode": str(args.signature_mode),
            "block_resolutions": [int(resolution) for resolution in block_resolutions],
            "atomic_width": int(atomic_width),
            ...
            "ot_output_paths": [
                str(
                    ...
                    / (
                        f"mcqa_plot_native_support_layer-{int(layer)}_pos-{DEFAULT_TOKEN_POSITION_ID}"
                        f"_atomic-{int(atomic_width)}_sig-{str(args.signature_mode)}_eps-{float(epsilon):g}_ot.json"
                    )
                )
                for epsilon in ot_epsilons
            ],
            ...
            "localization_runtime_seconds": float(layer_total_seconds),
