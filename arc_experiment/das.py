"""DAS search over ARC Gemma residual-stream sites."""
...
from .data import ARCPairBank, ARCPairDataset
...
class DASConfig:
    """Hyperparameters controlling ARC DAS search."""
...
def train_das_candidate(
    *,
    ...
    loader = DataLoader(ARCPairDataset(bank), batch_size=batch_size, shuffle=True)
    losses: list[float] = []
    best_loss = None
    plateau_steps = 0
...
def evaluate_das_candidate(
    *,
    ...
    loader = DataLoader(ARCPairDataset(bank), batch_size=batch_size, shuffle=False)
    logits_all = []
    for batch in loader:
        logits = run_das_residual_intervention(
            model=model,
...
def run_das_pipeline(
    *,
    model,
    train_bank: ARCPairBank,
    calibration_bank: ARCPairBank,
    holdout_bank: ARCPairBank,
