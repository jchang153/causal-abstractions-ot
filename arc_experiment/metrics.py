"""ARC-specific label extraction and reporting metrics."""
...
from .data import ARCPairBank, ALPHABET_LABELS
...
def gather_variable_logits(logits: torch.Tensor, bank: ARCPairBank) -> torch.Tensor:
    ...
    raise ValueError(f"Unsupported ARC target variable {bank.target_var}")


def cross_entropy_for_bank(logits: torch.Tensor, bank: ARCPairBank) -> torch.Tensor:
...
def metrics_from_logits(logits: torch.Tensor, bank: ARCPairBank, tokenizer=None) -> dict[str, float]:
...
def prediction_details_from_logits(logits: torch.Tensor, bank: ARCPairBank, tokenizer=None) -> dict[str, object]:
...
def build_variable_signature(bank: ARCPairBank, signature_mode: str) -> torch.Tensor:
    """Build the abstract-variable signature for one ARC target variable."""
