"""Transport-based alignment and intervention for ARC residual-stream sites."""
...
def run_alignment_pipeline(
    *,
    ...
    """Run OT or UOT for one ARC target variable."""
    device = torch.device(device)
    if config.selection_verbose:
        print(
    ...
    if config.method == "ot":
        ...
        raise ValueError(f"Unsupported ARC transport method {config.method}")
