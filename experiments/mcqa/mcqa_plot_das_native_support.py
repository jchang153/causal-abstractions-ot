...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--guided-mask-names",
        help="Compatibility flag. Native support DAS always uses the exact Selected PLOT handle.",
    )
    parser.add_argument("--guided-support-dim-count", type=int, default=DEFAULT_GUIDED_SUPPORT_DIM_COUNT)
    parser.add_argument(
        "--guided-subspace-dims",
        default=None,
        help="Compatibility flag. By default native support uses evenly spaced dims up to the selected support width.",
    )
...
def main() -> None:
    ...
    if native_resolution_value is None:
        raise KeyError(
            ...
            "'native_resolution' and compatibility 'atomic_width'."
        )
