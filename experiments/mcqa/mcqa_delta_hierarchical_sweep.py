...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            "Compatibility flag. PCA support now always keeps only the best calibrated PLOT handle."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--guided-mask-names",
        default="Selected",
        help="Compatibility flag for native-support DAS; native support always uses the exact Selected PLOT handle.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help="Compatibility flag for native-support DAS. Native support defaults to evenly spaced dims up to the selected support width.",
    )
    parser.add_argument(
...
def _build_stage_a_command(
    *,
    ...
    command = [
        sys.executable,
        "experiments/mcqa/mcqa_plot_layer.py",
        "--device",
        str(args.device),
        "--model-name",
        str(args.model_name),
...
def _build_stage_b_or_c_command(
    *,
    ...
    command = [
        sys.executable,
        "experiments/mcqa/mcqa_ot_pca_focus.py",
        "--device",
        str(args.device),
        "--dataset-path",
        str(args.dataset_path),
...
def _build_native_block_command(
    *,
    ...
    command = [
        sys.executable,
        "experiments/mcqa/mcqa_plot_native_support.py",
        "--device",
        str(args.device),
        "--model-name",
        str(args.model_name),
...
def _build_layer_das_command(
    *,
    ...
    command = [
        sys.executable,
        "experiments/mcqa/mcqa_plot_das_layer.py",
        "--device",
        str(args.device),
        "--model-name",
        str(args.model_name),
...
def _build_native_support_das_command(
    *,
    ...
    command = [
        sys.executable,
        "experiments/mcqa/mcqa_plot_das_native_support.py",
        "--device",
        str(args.device),
        "--model-name",
        str(args.model_name),
...
def _build_pca_support_das_command(
    *,
    args: argparse.Namespace,
    normalized: dict[str, object],
    stage_timestamp: str,
    pca_support_path: Path,
    target_vars: tuple[str, ...],
) -> tuple[str, ...]:
    command = [
        sys.executable,
        "experiments/mcqa/mcqa_plot_das_pca_support.py",
        "--device",
        str(args.device),
        "--model-name",
        str(args.model_name),
...
def main() -> None:
    ...
    if "stage_c_plot_das_pca_support" in normalized["stages"]:
        ...
        for entry in selected_config_entries:
            ...
            if target_var not in DEFAULT_TARGET_VARS:
                continue
            pca_support_path = Path(str(entry.get("layer_payload_path", "")))
            if not _stage_output_is_valid(pca_support_path):
                continue
            ...
            sweep_run_root = results_root / f"{stage_timestamp}_mcqa_plot_das_pca_support"
            site_catalog_tag = _site_catalog_tag(
                site_menu=str(site_menu),
                num_bands=int(num_bands),
                band_scheme=str(args.pca_band_scheme),
            ...
            summary_payload_path = (
                sweep_run_root
                / f"layer_{int(layer):02d}"
                / f"mcqa_plot_das_pca_support_layer-{int(layer)}_summary.json"
            )
            guided_payload_path = (
                sweep_run_root
                / f"layer_{int(layer):02d}"
                / (
                    f"mcqa_layer-{int(layer)}_pos-{str(token_position_id)}_pca-{site_catalog_tag}"
                    f"_basis-{str(basis_source_mode)}_sig-{str(args.signature_mode)}"
                    f"_{target_var}_das_guided.json"
                )
            )
            ...
            stage = SweepStage(
                ...
                description=(
                    f"Cached-support DAS on the selected PCA PLOT handle for {target_var}: "
                    f"token_position={token_position_id}, basis={basis_source_mode}, "
                    f"menu={site_menu}, bands={num_bands}, layer={int(layer)}."
                ),
                stage_timestamp=stage_timestamp,
                command=_build_pca_support_das_command(
                    args=args,
                    normalized=normalized,
                    stage_timestamp=stage_timestamp,
                    pca_support_path=pca_support_path,
                    target_vars=(target_var,),
                ),
                expected_outputs=tuple(expected_outputs),
            )
