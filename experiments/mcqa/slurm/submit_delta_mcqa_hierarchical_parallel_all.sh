...
mapfile -t STAGE_A_JOBS < <(
  SPLIT_SEED="${SPLIT_SEED}" bash experiments/mcqa/slurm/submit_delta_mcqa_hierarchical_parallel_stage_a.sh "${TIMESTAMP}" 2>&1 \
...
STAGE_B_SUBMITTER_JOB="$(
  sbatch \
    ...
    --wrap="cd ${PWD} && export HF_TOKEN=\"\$(cat \$HOME/.secrets/hf_token)\" && SPLIT_SEED=${SPLIT_SEED} SUBMIT_STAGE_C=${SUBMIT_STAGE_C} ARRAY_THROTTLE_STAGE_B=${ARRAY_THROTTLE_STAGE_B} ARRAY_THROTTLE_STAGE_C=${ARRAY_THROTTLE_STAGE_C} bash experiments/mcqa/slurm/delta_mcqa_hierarchical_parallel_after_stage_a.sh ${TIMESTAMP}" \
    | awk '/Submitted batch job/ {print $4}'
)"
...
if [[ "${SUBMIT_FULL_DAS}" == "1" ]]; then
  ...
  mapfile -t FULL_DAS_JOBS < <(
    SPLIT_SEED="${SPLIT_SEED}" bash experiments/mcqa/slurm/submit_delta_mcqa_full_das_timed.sh "${FULL_DAS_TIMESTAMP}" 2>&1 \
