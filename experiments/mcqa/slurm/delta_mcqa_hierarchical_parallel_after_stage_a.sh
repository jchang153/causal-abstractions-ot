...
TIMESTAMP="${1:?Usage: bash experiments/mcqa/slurm/delta_mcqa_hierarchical_parallel_after_stage_a.sh <timestamp>}"
...
mapfile -t STAGE_B_JOBS < <(
  SPLIT_SEED="${SPLIT_SEED}" ARRAY_THROTTLE="${ARRAY_THROTTLE_STAGE_B}" \
    bash experiments/mcqa/slurm/submit_delta_mcqa_hierarchical_parallel_stage_b.sh "${TIMESTAMP}" 2>&1 \
...
if [[ "${SUBMIT_STAGE_C}" != "1" ]]; then
  ...
  sbatch \
    ...
    --job-name=mcqa-agg \
    --wrap="cd ${PWD} && ALLOW_PARTIAL=1 bash experiments/mcqa/slurm/aggregate_delta_mcqa_hierarchical_parallel.sh ${TIMESTAMP}"
  exit 0
fi
...
sbatch \
  ...
  --wrap="cd ${PWD} && export HF_TOKEN=\"\$(cat \$HOME/.secrets/hf_token)\" && SPLIT_SEED=${SPLIT_SEED} bash experiments/mcqa/slurm/delta_mcqa_hierarchical_parallel_after_stage_b.sh ${TIMESTAMP}"
