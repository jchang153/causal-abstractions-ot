...
TIMESTAMP="${1:?Usage: bash experiments/mcqa/slurm/submit_delta_mcqa_hierarchical_parallel_stage_c.sh <timestamp>}"
...
COMMON_ARGS=(
  ...
  --calibration-metric "${CALIBRATION_METRIC:-iia_acc}"
  ...
  --pca-num-bands-values "${PCA_NUM_BANDS_VALUES:-1,2,4,8,16,32,64}"
...
python experiments/mcqa/mcqa_delta_hierarchical_parallel.py aggregate-stage-b --skip-native-aggregation "${COMMON_ARGS[@]}"
python experiments/mcqa/mcqa_delta_hierarchical_parallel.py plan-stage-c "${COMMON_ARGS[@]}"
...
sbatch \
  ...
  experiments/mcqa/slurm/delta_mcqa_hierarchical_parallel_task.sbatch
