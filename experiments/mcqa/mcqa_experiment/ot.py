...
def load_prepared_alignment_artifacts(
    ...
    *,
    ...
    if not isinstance(base_logits_by_var, dict) or not isinstance(site_signatures_by_var, dict):
        compatibility_base_logits = payload.get("base_logits")
        compatibility_site_signatures = payload.get("site_signatures")
        if isinstance(compatibility_base_logits, torch.Tensor) and isinstance(compatibility_site_signatures, torch.Tensor):
            ...
            base_logits_by_var = {target_var: compatibility_base_logits.detach().cpu()}
            site_signatures_by_var = {target_var: compatibility_site_signatures.detach().cpu()}
        else:
            return None
...
def solve_ot_transport(
    ...
    if isinstance(variable_signatures_by_var, torch.Tensor) and isinstance(site_signatures_by_var, torch.Tensor):
        cost_cross = torch.cdist(
            variable_signatures_by_var.to(torch.float32),
            site_signatures_by_var.to(torch.float32),
        ).pow(2)
    else:
...
def run_alignment_pipeline(
    *,
    model,
    fit_banks_by_var: dict[str, MCQAPairBank] | None = None,
    calibration_bank: MCQAPairBank | None = None,
    holdout_bank: MCQAPairBank | None = None,
    sites: list[SiteLike],
    device: torch.device | str,
    tokenizer,
    config: OTConfig,
    ...
    pca_bases_by_id: dict[str, LayerPCABasis] | None = None,
    fit_banks: dict[str, MCQAPairBank] | None = None,
    ...
    if calibration_bank is None and calibration_banks:
        first_key = next(iter(calibration_banks))
        calibration_bank = calibration_banks[first_key]
    if holdout_bank is None and holdout_banks:
        first_key = next(iter(holdout_banks))
        holdout_bank = holdout_banks[first_key]
    if fit_banks_by_var is None or calibration_bank is None or holdout_bank is None:
        raise ValueError("fit_banks_by_var, calibration_bank, and holdout_bank are required")
    device = torch.device(device)
