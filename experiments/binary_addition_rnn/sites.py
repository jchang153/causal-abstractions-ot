...
@dataclass(frozen=True)
class RotatedGroupSite:
    timestep: int
    component_indices: tuple[int, ...]
    basis_name: str = "pca"

    def key(self) -> str:
        joined = ",".join(str(i) for i in self.component_indices)
        return f"h_{self.timestep}[{self.basis_name}:{joined}]"
...
def enumerate_rotated_group_sites_for_timesteps(
    *,
    timesteps: tuple[int, ...],
    hidden_size: int,
    resolution: int,
    basis_name: str = "pca",
) -> tuple[RotatedGroupSite, ...]:
    resolution = int(resolution)
    hidden_size = int(hidden_size)
    if resolution <= 0 or resolution > hidden_size:
        raise ValueError(f"resolution must be in [1, {hidden_size}], got {resolution}")
...
def enumerate_rotated_prefix_sites_for_timesteps(
    *,
    timesteps: tuple[int, ...],
    hidden_size: int,
    resolution: int,
