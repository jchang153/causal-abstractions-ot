...
from .sites import CoordinateGroupSite, FullStateSite, NeuronSite, OutputLogitSite, RotatedGroupSite, Site

if TYPE_CHECKING:
    from .pca_basis import RotatedBasis
...
def _apply_site_delta(
    ...
    *,
    weight: float,
    lambda_scale: float,
    rotation_map: dict[int, "RotatedBasis"] | None = None,
) -> torch.Tensor:
    ...
    if isinstance(site, CoordinateGroupSite):
        ...
        updated[:, idx] = updated[:, idx] + float(lambda_scale) * float(weight) * (src_h[:, idx] - updated[:, idx])
        return updated
    if isinstance(site, RotatedGroupSite):
        if rotation_map is None or int(site.timestep) not in rotation_map:
            raise ValueError(f"rotation_map is required for rotated site {site.key()}")
        basis = rotation_map[int(site.timestep)]
        ...
        z_src = ((src_h - mean.unsqueeze(0)) @ rotation) / scale.unsqueeze(0)
        z_int = z_base.clone()
        idx = list(site.component_indices)
        z_int[:, idx] = z_int[:, idx] + float(lambda_scale) * float(weight) * (z_src[:, idx] - z_int[:, idx])
