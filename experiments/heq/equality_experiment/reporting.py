"""Console reporting helpers for equality experiment summaries."""
...
def print_results_table(records: list[dict[str, object]], title: str) -> None:
    """Print a compact table of per-variable equality results."""
    print(title)
    if not records:
        print("(no records)")
        return
...
def summarize_method_records(records: list[dict[str, object]]) -> list[dict[str, object]]:
    """Average per-variable exact accuracy into one summary per method."""
...
def build_method_selection_summary(method: str, payload: dict[str, object]) -> dict[str, object]:
    ...
    if method in {"gw", "ot", "uot", "fgw", "cosine", "bruteforce"}:
        return {
            "method": method,
...
def format_method_selection_summary(summary: dict[str, object]) -> str:
    """Format one compact equality-method summary as human-readable text."""
...
def format_method_candidate_sweep(method: str, payload: dict[str, object]) -> str:
    ...
    if method in {"gw", "ot", "uot", "fgw", "cosine"}:
            ...
            for variable in payload.get("target_vars", []):
                ...
                for candidate in [*selected_candidates, *remaining_candidates]:
                    ...
                    lines.append(
                        ...
                        f"cutoff={candidate.get('continuous_cutoff')}, "
                        ...
                        f"cal_ce={candidate.get('selection_cross_entropy')}, "
