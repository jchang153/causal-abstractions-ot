...
RUN_DIR = Path("results") / f"{RUN_TIMESTAMP}_equality_sweep"
...
RESOLUTION = 1
FGW_ALPHA = 0.5
OT_EPSILONS = (0.005, 0.01, 0.03, 0.05, 0.1, 0.2)
OT_TAUS = (0.1, 0.25, 0.5, 1.0, 2.0, 4.0)
UOT_REG_MS = (0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.0, 4.0)
OT_TOP_K_VALUES = tuple(range(1, 21))
OT_LAMBDAS = tuple(np.arange(0.1, 4.0 + 1e-9, 0.1))
...
def build_compare_config(run_dir: Path, methods: tuple[str, ...], ot_epsilon: float, ot_tau: float, uot_reg_m: float, signature_mode: str) -> CompareExperimentConfig:
...
def sweep_record(method: str, epsilon: float, tau: float, uot_reg_m: float, comparison: dict[str, object], run_dir: Path, signature_mode: str) -> dict[str, object]:
...
def main() -> None:
    ...
    das_dir = RUN_DIR / "das_fixed"
    das_comparison = run_comparison_with_banks(
        model=model,
        backbone_meta=backbone_meta,
        device=device,
        config=build_compare_config(das_dir, ("das",), OT_EPSILONS[0], OT_TAUS[0], UOT_REG_MS[0], SIGNATURE_MODES[0]),
    ...
    total = len(SIGNATURE_MODES) * len(OT_EPSILONS) * len(OT_TAUS) * sum(len(reg_ms) for _, reg_ms in sweep_methods)
            ...
            for tau in OT_TAUS:
                for method, reg_ms in sweep_methods:
                    for uot_reg_m in reg_ms:
                        sweep_index += 1
                        tag = f"{method}_{signature_mode}_eps{str(epsilon).replace('.', 'p')}_tau{str(tau).replace('.', 'p')}_m{str(uot_reg_m).replace('.', 'p')}"
                        run_dir = RUN_DIR / tag
                        print(f"[{sweep_index}/{total}] method={method} signature={signature_mode} epsilon={epsilon} tau={tau} uot_reg_m={uot_reg_m}")
                        comparison = run_comparison_with_banks(
                            model=model,
                            backbone_meta=backbone_meta,
                            device=device,
                            config=build_compare_config(run_dir, (method,), epsilon, tau, uot_reg_m, signature_mode),
                            train_bank=train_bank,
                            calibration_bank=calibration_bank,
                            test_bank=test_bank,
                        )
                        record = sweep_record(method, epsilon, tau, uot_reg_m, comparison, run_dir, signature_mode)
    ...
    payload = {
        ...
        "das_fixed": das_comparison,
        "transport_sweep": transport_records,
        "best_by_method": best_by_method,
    }
    write_json(RUN_DIR / "equality_sweep_results.json", payload)

    summary_lines = [
        "Equality Sweep Summary",
        f"run_dir: {RUN_DIR}",
        f"seed: {SEED}",
    ...
    for method in ("gw", "fgw", "ot", "uot"):
        rec = best_by_method.get(method)
        if rec is None:
            continue
        summary_lines.extend(
            [
                f"{method.upper()}: avg={float(rec['average_exact_acc']):.4f} | WX={float(rec['per_variable_exact_acc']['WX']):.4f} | YZ={float(rec['per_variable_exact_acc']['YZ']):.4f} | sig={rec['signature_mode']} | eps={float(rec['epsilon']):.4f} | tau={float(rec['tau']):.4f} | reg_m={float(rec['uot_reg_m']):.4f}",
            ]
        )
    write_text_report(RUN_DIR / "equality_sweep_summary.txt", "\n".join(summary_lines))
