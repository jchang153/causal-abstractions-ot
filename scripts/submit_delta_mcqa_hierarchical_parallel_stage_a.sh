...
echo "[submit-delta-hpar-a] stage_a_time=${DELTA_STAGE_A_TIME}"
echo "[submit-delta-hpar-a] stage_a_job_name=${STAGE_A_JOB_NAME}"
...
echo "[submit-delta-hpar-a] stage_a_rerank_drop_ratio=${STAGE_A_RERANK_DROP_RATIO:-0}"
echo "[submit-delta-hpar-a] stage_a_rerank_min_k=${STAGE_A_RERANK_MIN_K:-6}"
echo "[submit-delta-hpar-a] stage_a_rerank_max_k=${STAGE_A_RERANK_MAX_K:-8}"
echo "[submit-delta-hpar-a] uot_beta_neurals=${UOT_BETA_NEURALS:-0.03,0.1,0.3,1,3}"
echo "[submit-delta-hpar-a] batch_size=${BATCH_SIZE:-64}"
echo "[submit-delta-hpar-a] hf_home=${HF_HOME}"
echo "[submit-delta-hpar-a] sbatch_extra_args=${SBATCH_EXTRA_ARGS}"
echo "[submit-delta-hpar-a] split_seed=${SPLIT_SEED}"

SBATCH_ARGS=(
  --account="${DELTA_ACCOUNT}"
  --partition="${DELTA_PARTITION}"
  --time="${DELTA_STAGE_A_TIME}"
  --export=ALL
