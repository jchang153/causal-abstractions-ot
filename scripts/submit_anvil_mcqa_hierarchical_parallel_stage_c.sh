...
TIMESTAMP="${1:?Usage: bash scripts/submit_anvil_mcqa_hierarchical_parallel_stage_c.sh <timestamp>}"
...
ANVIL_STAGE_C_TIME="${ANVIL_STAGE_C_TIME:-02:00:00}"
...
echo "[submit-anvil-hpar-c] timestamp=${TIMESTAMP}"
echo "[submit-anvil-hpar-c] ANVIL_ACCOUNT=${ANVIL_ACCOUNT}"
echo "[submit-anvil-hpar-c] ANVIL_PARTITION=${ANVIL_PARTITION}"
echo "[submit-anvil-hpar-c] array_throttle=${ARRAY_THROTTLE}"
echo "[submit-anvil-hpar-c] stage_c_time=${ANVIL_STAGE_C_TIME}"
echo "[submit-anvil-hpar-c] stage_a_method=${STAGE_A_METHOD:-ot}"
echo "[submit-anvil-hpar-c] uot_beta_neurals=${UOT_BETA_NEURALS:-0.03,0.1,0.3,1,3}"
echo "[submit-anvil-hpar-c] regular_das_subspace_dims=${REGULAR_DAS_SUBSPACE_DIMS:-32,64,96,128,256,512,768,1024,1536,2048,2304}"
echo "[submit-anvil-hpar-c] guided_subspace_dims=${GUIDED_SUBSPACE_DIMS:-32,64,96,128,256,512,768,1024,1536,2048,2304}"
echo "[submit-anvil-hpar-c] split_seed=${SPLIT_SEED}"
...
if [[ "${STAGE_C_TASKS}" -le 0 ]]; then
  echo "[submit-anvil-hpar-c] no Stage C tasks"
  exit 0
fi

ARRAY_SPEC="0-$((STAGE_C_TASKS - 1))%${ARRAY_THROTTLE}"
echo "[submit-anvil-hpar-c] submitting Stage C tasks=${STAGE_C_TASKS} array=${ARRAY_SPEC}"
sbatch \
  --account="${ANVIL_ACCOUNT}" \
  --partition="${ANVIL_PARTITION}" \
  --array="${ARRAY_SPEC}" \
  --time="${ANVIL_STAGE_C_TIME}" \
  --export=ALL,TASK_FILE="${STAGE_C_TASK_FILE}" \
  --job-name=mcqa-hpar-c \
  --output="${ROOT}/mcqa-hpar-c-%A_%a.out" \
  --error="${ROOT}/mcqa-hpar-c-%A_%a.err" \
  scripts/anvil_mcqa_hierarchical_parallel_task.sbatch

echo "[submit-anvil-hpar-c] task plan: ${ROOT}/stage_c_task_plan.txt"
