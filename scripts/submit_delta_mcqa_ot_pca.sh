...
echo "[submit-delta-ot-pca] top_prefix_sizes=${TOP_PREFIX_SIZES:-8,16,32,64}"
...
echo "[submit-delta-ot-pca] screen_mask_names=${SCREEN_MASK_NAMES:-Top1,Top2}"
...
for LAYER in "${LAYERS[@]}"; do
  ...
  TOP_PREFIX_SIZES="${TOP_PREFIX_SIZES:-8,16,32,64}" \
  BASIS_SOURCE_MODE="${BASIS_SOURCE_MODE:-all_variants}" \
  SCREEN_DAS="${SCREEN_DAS:-0}" \
  SCREEN_MASK_NAMES="${SCREEN_MASK_NAMES:-Top1,Top2}" \
